...
class Heterogeneous1DReactorSaver(BasicSaver):
    def __init__(self, cls):
        """
        Class to save HETEROGENEOUS 1D REACTOR results
    ...
    def species_mass_fraction_to_df(self, species_names):
        ...
        if self.cls.solution_parser.resolution_method == ResolutionMethod.STEADYSTATE:
            ...
            output_dict = self.create_df_from_x_vector_and_y_matrix(self.x,
                                                                    ym,
            ...
            output_dict.update(self.create_df_from_x_vector_and_y_matrix(self.x,
                                                                          ym,
                                                                          self._variables_name.length,
        ...
        if self.cls.solution_parser.resolution_method == ResolutionMethod.TRANSIENT:
            ...
            for s in species_names:
                ...
                sheet_name = f"{self._variables_name.solid_mass_fraction} {s}"
                output_dict.update(self.create_df_from_x_vector_and_y_matrix(self.length,
                                                                             ym,
                                                                             self._variables_name.length,
                                                                             sheet_name,
    ...
    def species_mole_fraction_to_df(self, species_names):
        ...
        if self.cls.solution_parser.resolution_method == ResolutionMethod.TRANSIENT:
            ...
            for s in species_names:
                ...
                sheet_name = f"{self._variables_name.solid_mole_fraction} {s}"
                output_dict.update(self.create_df_from_x_vector_and_y_matrix(self.length,
                                                                             ym,
                                                                             self._variables_name.length,
                                                                             sheet_name,
    ...
    def temperature_to_df(self):
        ...
        """
        if self.cls.solution_parser.resolution_method == ResolutionMethod.STEADYSTATE:
            output_dict = self.create_df_from_x_vector_and_y_vector(self.x,
                                                                    self.temperature,
                                                                    self._variables_name.length,
                                                                    self._variables_name.gas_temperature)
            output_dict.update(self.create_df_from_x_vector_and_y_vector(self.x,
        ...
        if self.cls.solution_parser.resolution_method == ResolutionMethod.TRANSIENT:
            ...
            output_dict = self.create_df_from_x_vector_and_y_matrix(self.length,
                                                                    ym,
                                                                    self._variables_name.length,
                                                                    self._variables_name.gas_temperature,
                                                                    column_names)
